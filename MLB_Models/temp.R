...
setup.model = function(hitters, pitchers, lineups, num.overlap,
  ...
  for(i in 1:nrow(lineups)) {
    overlap.constraints = append(lineups[i, 1:(num.hitters + num.pitchers)],
  ...
  # No team can be chosen more than 15 times
  # for(i in 1:num.teams) {
  #   lhs                  = rep(0, nCol)
  #   shift                = num.hitters + num.pitchers
  #   lhs[shift + i]       = 1
  #   constraints          = list.append(constraints, lhs)
  #   model$sense          = append(model$sense, c("<"))
  #   model$rhs            = append(model$rhs, c(15 - sum(lineups[,(shift + i)])))
  # }
...
solve.model = function(model, num.hitters,
  ...
  for(i in 1:length(result$pool)) {
    df = list.append(df, result$pool[[i]]$xn)
  }
  
  df = unique(df)
  
  indexes = list(which(df[[1]][1:(num.hitters + num.pitchers)] > 0.5))
  chosen = c(1)
  
  for(i in 2:length(df)) {
    lineup = df[[i]][1:(num.hitters + num.pitchers)]
    tryCatch({
      temp = which(lineup > 0.5)
      ...
      chosen = c(chosen, i)
      indexes = list.append(indexes, 
                            which(df[[i]][1:(num.hitters + num.pitchers)] > 0.5))
    }, error = function(e) {})
  }
  
  df = df[chosen]
  
  return(matrix(unlist(df), ncol=length(df[[1]]), byrow=TRUE))
}
...
create_lineups = function(num.lineups, num.overlap, formulation, objective,
  ...
  print(nrow(lineups))
  if(num.lineups > 1) {
    while(nrow(lineups) < num.lineups) {
  ...
  lineups = lineups[,1:(num.hitters + num.pitchers)]
  return(lineups)
}
      ...
      tryCatch({
        ...
        for(a in c(0.2, 0.4, 0.6, 0.8)) {
          for(b in c(0.2, 0.4, 0.6, 0.8)) {
            df1   = create_lineups(num.lineups, num.overlap, setup.model, objective.a,
                                   salary.cap, hitters.proj, pitchers.proj, a, b, 
                                   c, d, T)
            
            scores = get.scores(df1, hitters.proj, pitchers.proj)
